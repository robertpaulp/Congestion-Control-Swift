...
public:
    ...
    inline static CCAck* newpkt(PacketFlow &flow, const Route &route, seq_t ackno, simtime_picosec ts, bool ecn) {
    ...
    inline simtime_picosec ts() const {return _ts;}
    inline void set_ts(simtime_picosec ts) {_ts = ts;}
    inline void set_ecn(bool ecn) {_ecn_echo = ecn;}
    inline bool is_ecn_marked() const {return _ecn_echo;}
