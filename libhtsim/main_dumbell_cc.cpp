...
int main(int argc, char **argv) {
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")) {
            ...
            ecnmin = atoi(argv[i+1]);
            ecnmax = atoi(argv[i+1]);	    
            cout<< "ECN thresholds set to " << ecnmin << " " << ecnmax << endl;
            i+=2;
    ...
    ecnmin = memFromPkt(ecnmin);
    ecnmax = memFromPkt(ecnmax);
    ...
    queue.set_ecn_thresholds(ecnmin, ecnmax);
    queue2.set_ecn_thresholds(ecnmin, ecnmax);
